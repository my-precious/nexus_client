...
async fn main() {
    ...
    let mut config_path = String::from("./nexus_client.txt");
    let mut i = 1;
    while i < args.len() {
    ...
    let mut file = File::open(&config_path).unwrap_or_else(|e| {
        panic!("无法打开配置文件 {}: {}", config_path, e);
    });
    ...
    log::info!("钱包地址: {}", config.address);

    log::info!("分布式计算节点启动！");
    let pick_url = format!("http://{}:{}/wallets/{}/tasks/pick", config.host, config.port, config.address);
    let submit_url = format!("http://{}:{}/wallets/{}/tasks/submit", config.host, config.port, config.address);
