...
# 客户端配置文件名
CONFIG_FILE="$CLIENT_PATH/nexus_client88.txt"

pkill -f nexus_client88

for ((i=1; i<=NUM_CLIENTS; i++))
do
  if [ -f "$CONFIG_FILE" ]; then
    echo "启动第 $i 个客户端，配置文件: $CONFIG_FILE"
    rm -rf ~/.nexus/client88_${i}.log
    $CLIENT_PATH/target/release/nexus_client --file "$CONFIG_FILE" > ~/.nexus/client88_${i}.log 2>&1 &
  else
